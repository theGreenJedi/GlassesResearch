...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (request.method === 'GET' && url.pathname === '/health') return json({ok:true, service:'verified-research-alerts'});
...
async function ingestPublished(request,env,ctx){const auth=request.headers.get('authorization')||'';if(!env.PUBLISH_TOKEN || auth!==`Bearer ${env.PUBLISH_TOKEN}`)return json({ok:false},401);const p=await request.json();if(!p.id||!p.title||!p.canonical_url||!String(p.canonical_url).startsWith(env.SITE_ORIGIN))return json({ok:false,message:'Invalid publication payload.'},400);const item={id:String(p.id),title:String(p.title),canonical_url:String(p.canonical_url),summary:String(p.summary||''),models:normalizeList(p.models),brands:normalizeList(p.brands_lineages||p.brands),topics:normalizeList(p.topics).filter(x=>TOPICS.has(x)),published_at:p.published_at||now()};await env.DB.prepare(`INSERT INTO published_items(id,title,canonical_url,summary,models_json,brands_json,topics_json,published_at,created_at) VALUES(?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET title=excluded.title,canonical_url=excluded.canonical_url,summary=excluded.summary,models_json=excluded.models_json,brands_json=excluded.brands_json,topics_json=excluded.topics_json,published_at=excluded.published_at`).bind(item.id,item.title,item.canonical_url,item.summary,JSON.stringify(item.models),JSON.stringify(item.brands),JSON.stringify(item.topics),item.published_at,now()).run();ctx.waitUntil(deliverAsVerified(item,env));return json({ok:true})}
...
async function sendMail(env,message){if(!env.RESEND_API_KEY)throw new Error('RESEND_API_KEY is not configured');const body={from:env.MAIL_FROM,to:[message.to],subject:message.subject,text:message.text,html:message.html,headers:message.headers||{}};const r=await fetch('https://api.resend.com/emails',{method:'POST',headers:{authorization:`Bearer ${env.RESEND_API_KEY}`,'content-type':'application/json'},body:JSON.stringify(body)});if(!r.ok)throw new Error(`Mail provider returned ${r.status}: ${await r.text()}`)}
