...
- Xiaomi AI Glasses: **source boundary completed in EV-0067** — local camera/audio/storage and low-power Vela architecture are separated from still-unknown account/app gates, media access, AI processing location, offline translation/transcription, regional behavior and recovery. Remaining: supported-region vs clean-phone hands-on activation, sign-out, export, endpoint-blocking, firmware and reset tests.
...
- RealWear HMT-1/HMT-1Z1: **source boundary completed in EV-0037 and structured records GLS-0101/0102** — RealWear explicitly says EOL devices still work, firmware/security end at 12.6, and English/German/Mandarin dictation remains local while other languages use cloud dictation. Remaining: reset, app deployment, MDM/account, third-party-app and endpoint-blocked tests.
- HoloLens 1/2: **source boundary completed in EV-0066** — HoloLens 1 continued function and local sideloading are separated from ended security support; HoloLens 2 default network/account OOBE is separated from local-account offline provisioning, USB/Device Portal app deployment and pre-downloaded FFU recovery. Remaining: post-2027 sign-out, Store, certificate, reset and endpoint-blocked tests.
...
- Ampere Dusk Rx and Chamelo Aura Rx: EV-0070 establishes **vendor-integrated prescription configurations with independent optical service unverified**; Dusk Rx publishes approximately ±7 SPH/±2 CYL while sold out, and Aura Rx has a current vendor ordering path but no primary correction range or independent-reglazing proof.
- Iristick G3: EV-0069 supports **ordinary-optician service through an owner-removable magnetic insert**; exact correction/progressive limits, material requirements, price/stock and safety-certification effects remain unknown.
- HeyCyan/W610: EV-0068 supports a **prescription-compatible supplier claim only**; Goodway advertises removable/customizable prescription lenses for OEM/ODM buyers, while the exact retail variant, correction limits, replacement supply and ordinary-optician path remain unverified.
...
First-wave source boundaries are complete. Meta/Ray-Ban/Oakley, Even G1/G2, Brilliant Frame/Halo, Rokid Glasses and RayNeo X3 Pro are covered in EV-0049/EV-0050; W610 has a source boundary in EV-0068 but still requires the hands-on closure protocol. Remaining optical work is evidence enrichment: exact ranges, progressive support, replacement availability, regional variance and ordinary-independent service verification.
