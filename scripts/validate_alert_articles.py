#!/usr/bin/env python3
"""Require every dispatch-enabled GRE-derived alert to resolve to a real site article."""
...
def main() -> int:
    ...
    for item in manifest.get("publications", []):
        ...
        event_id = item.get("event_id", "<unknown GRE>")
        url = str(item.get("canonical_url", ""))
        try:
            path = article_path(url)
        except ValueError as exc:
            errors.append(f"{event_id}/{publication_id}: {exc}")
            continue
        if not path.is_file():
            errors.append(f"{event_id}/{publication_id}: article file does not exist: {path.relative_to(ROOT)}")
            continue
        text = path.read_text(encoding="utf-8")
        title = str(item.get("title", "")).strip()
        if title and title not in text:
            errors.append(f"{event_id}/{publication_id}: article does not contain ledger title")
    if errors:
        raise SystemExit("\n".join(errors))
    print(f"Article-backed GRE alert contract valid: {dispatch_count} dispatch-enabled publications")
    return 0
