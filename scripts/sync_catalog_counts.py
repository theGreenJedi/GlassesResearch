...
Admission decisions are established upstream in dated reconciliation packets. A packet
may be human-reviewed or generated by the bounded automatic canonical-admission lane;
this synchronizer never decides identity or admission itself. It performs only
mechanical propagation:
- insert missing GLS rows already authorized in THE_LIST_RECONCILIATION_*.md packets;
- preserve an explicit Era column when a reconciliation packet provides one;
...
def packet_admissions() -> list[tuple[str, str, bool]]:
    """Return (GLS id, canonical markdown row, explicit-era) for authorized rows."""
