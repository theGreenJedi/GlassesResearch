...
from cloudflare_rum import rum_period
...
def question_analytics(token, site, start, end):
    """Measure the FAQ / practical-answer corpus as one search-discovery cohort."""
    body = gsc_query(token, site, start, end, ["page", "query"], 25000)
    rows = []
    for row in body.get("rows", []):
        keys = row.get("keys") or ["", ""]
        page, query = keys[0], keys[1]
        if "/docs/faq/" not in page:
            continue
        rows.append({"page": page, "query": query, "clicks": float(row.get("clicks", 0)), "impressions": float(row.get("impressions", 0)), "ctr": float(row.get("ctr", 0)), "position": float(row.get("position", 0))})
    ...
    pages=sorted({r["page"] for r in rows if r["impressions"]})
    queries=sorted(rows,key=lambda r:(-r["clicks"],-r["impressions"],r["position"]))[:25]
    return {"start":start.isoformat(),"end":end.isoformat(),"clicks":clicks,"impressions":impressions,"ctr":clicks/impressions if impressions else 0.0,"position":weighted/impressions if impressions else 0.0,"pages_with_impressions":len(pages),"top_question_queries":queries}
...
def collect_gsc():
    ...
    try:
        ...
        questions_current = question_analytics(token, site, current_start, end)
        questions_previous = question_analytics(token, site, previous_start, previous_end)
        questions_month = question_analytics(token, site, month_start, end)
        return {
            "available": True,
            "property": site,
            ...
            "rolling_28d": month,
            "questions": {"current_7d": questions_current, "previous_7d": questions_previous, "rolling_28d": questions_month},
        }
    except Exception as exc:
...
def collect_cloudflare_web_analytics():
    ...
    try:
        return {
            "available": True,
            "rolling_24h": rum_period(token, zone, 24),
            "rolling_7d": rum_period(token, zone, 24 * 7),
            "note": (
                "Browser-side Cloudflare Web Analytics / RUM. Pageviews are browser beacon page loads; "
                "visits are Cloudflare RUM visits. These are not guaranteed unique people."
            ),
        }
    except Exception as exc:
        return {"available": False, "error": str(exc)}
...
def render(report):
    ...
    if g.get("available"):
        ...
        q = g["questions"]; qc=q["current_7d"]; qp=q["previous_7d"]; qm=q["rolling_28d"]
        lines += ["", "## Practical Answers — search discovery", "",
            f"- Latest 7 days: **{qc['impressions']:g} impressions**, **{qc['clicks']:g} clicks**, **{qc['ctr']*100:.2f}% CTR**, average position **{qc['position']:.1f}**, across **{qc['pages_with_impressions']} answer pages receiving impressions**.",
            f"- Week-over-week: impressions {fmt_delta(pct_delta(qc['impressions'], qp['impressions']))}; clicks {fmt_delta(pct_delta(qc['clicks'], qp['clicks']))}.",
            f"- Rolling 28 days: **{qm['impressions']:g} impressions**, **{qm['clicks']:g} clicks**, **{qm['pages_with_impressions']} answer pages receiving impressions**.",
            "", "### Queries reaching Practical Answers", "", md_table(qc["top_question_queries"], "query"),
            "## Google — top queries", "", md_table(cur["top_queries"], "query"), "## Google — top pages", "", md_table(cur["top_pages"], "page")]
    else:
        lines += ["## Executive summary", "", f"- **Google Search Console unavailable:** {g.get('error', 'unknown error')}", ""]

    w = report["cloudflare_web_analytics"]
    lines += ["## Cloudflare Web Analytics — browser traffic", ""]
    if w.get("available"):
        w24 = w["rolling_24h"]
        w7 = w["rolling_7d"]
        lines += [
            f"- Rolling 24h: **{w24['pageviews']:,} browser pageviews**, **{w24['visits']:,} RUM visits**.",
            f"- Rolling 7d: **{w7['pageviews']:,} browser pageviews**, **{w7['visits']:,} RUM visits**.",
        ...
        for row in w24["paths"][:10]:
            path = str(row["path"]).replace("|", "\\|")
            lines.append(f"| {path} | {row['pageviews']:,} | {row['visits']:,} |")
        lines += ["", "### Top referrers — rolling 24h", "", "| Referrer | Pageviews | RUM visits |", "|---|---:|---:|"]
        for row in w24["referrers"][:10]:
            ref = str(row["referrer"] or "(direct / none)").replace("|", "\\|")
            lines.append(f"| {ref} | {row['pageviews']:,} | {row['visits']:,} |")
        lines.append("")
    else:
        lines += [
            f"- **Cloudflare Web Analytics unavailable:** {w.get('error', 'unknown error')}",
            "- Edge analytics still runs independently.",
            "",
        ]
...
def main():
    report = {
        ...
        "cloudflare_web_analytics": collect_cloudflare_web_analytics(),
    ...
    if (
        not report["google_search_console"].get("available")
        and not report["cloudflare_web_analytics"].get("available")
        and not report["cloudflare"].get("available")
    ):
        return 1
    return 0
