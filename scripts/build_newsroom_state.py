#!/usr/bin/env python3
"""Build the reader-facing newsroom state from verified changes and editorial leads.

The verified desk remains derived only from the canonical verified-change ledger. Published
first-party editorials may lead the reader-facing presentation without becoming verified-change
events. An explicit editorial lead pin may feature a reviewed external article without changing
its verification state. Lead stories expire after seven days.
"""
...
def choose_lead(events: list[dict]) -> dict | None:
    ...
    recent = sorted(
        eligible,
        key=lambda e: stamp(e["publication"]["published_at"]),
        reverse=True,
    )[:LEAD_WINDOW]
    if not recent:
        return None
...
def load_first_party_editorial(article_dir: Path) -> dict | None:
    ...
    for path in sorted(article_dir.glob("*.md")):
        ...
        if not re.search(
            r"^\*\*Type:\*\*\s*GlassesResearch editorial(?:\s*/.*)?\s*$",
            text,
        ...
        if not title_match or not published_match:
            raise SystemExit(
                f"Editorial article lacks title or Published date: {path.relative_to(ROOT)}"
            )
        ...
        try:
            ...
            raise SystemExit(
                f"Editorial Published date must be 'Month D, YYYY': {path.relative_to(ROOT)}"
            ) from exc
        ...
        if not summary:
            raise SystemExit(
                f"Editorial article lacks a reader-facing summary: {path.relative_to(ROOT)}"
            )
...
def convergence(events: list[dict]) -> list[dict]:
    ...
    for (kind, key), grouped in buckets.items():
        ...
        label = (
            key
            if kind == "entity"
            else TOPIC_LABELS.get(key, key.replace("_", " ").title())
        )
        themes.append(
            {
                "kind": kind,
...
def build(
    ...
    for candidate in (first_party_editorial, editorial_lead):
        if candidate and (
            lead is None
            or stamp(candidate["published_at"]) >= stamp(lead["published_at"])
        ):
            lead = candidate
    ...
    return {
        ...
        "semantics": (
            "Verified desk state is derived only from verified published changes. "
            "A published first-party editorial or explicit reviewed external editorial pin may "
            "lead presentation without entering the verified desk. No lead may be more than seven days old."
        ),
...
def main() -> int:
    ...
    state = build(
        ...
        first_party_editorial=first_party_editorial,
    )
    ...
    lead_name = (
        state["lead"].get("event_id") or state["lead"].get("source_label", "none")
        if state["lead"]
        else "none"
    )
    print(
