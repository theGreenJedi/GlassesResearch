---
title: "Reference Desk"
description: "Fast paths for journalists, researchers, developers, and community writers to verify, cite, reuse, and challenge GlassesResearch smart-glasses research."
---

# Reference desk

Use this page when you need a verifiable smart-glasses fact, a stable model identity, a reusable dataset, or a citation-ready research link.

## Find the canonical model

- [Browse the complete canonical catalog](https://glassesresearch.org/models/catalog/)
- [Resolve a GLS identifier](https://glassesresearch.org/gls/)
- [Use the Glasses Finder](/docs/COMPARISON_ENGINE/) to filter and compare models
- [Check retail aliases and rebrands](/models/RETAIL_REBRANDS/)

Every canonical model has a stable `GLS-####` identifier. Use that identifier when a product has multiple seller names, rebrands, or changing market labels.

## Cite a model, verified publication, or dataset

- [How to Cite GlassesResearch](CITING_GLASSESRESEARCH.md)
- Per-model BibTeX: `https://glassesresearch.org/data/citations/gls-####.bib`
- Per-model CSL-JSON: `https://glassesresearch.org/data/citations/gls-####.json`
- Model citation index: `https://glassesresearch.org/data/citations/index.json`
- Aggregate model BibTeX: `https://glassesresearch.org/data/citations/glassesresearch-models.bib`
- Verified-research citation index: `https://glassesresearch.org/data/citations/verified-research/index.json`
- Aggregate verified-research BibTeX: `https://glassesresearch.org/data/citations/verified-research/glassesresearch-verified-research.bib`
- Canonical public model dataset: `https://glassesresearch.org/data/public/models.json`
- Flat catalog CSV: `https://glassesresearch.org/data/public/models.csv`

Standalone verified Research & News articles expose BibTeX and CSL-JSON directly. For a specific factual claim, follow the publication or model page to the underlying source and cite that primary source as well when practical.

## Reuse a model card

[Embeddable model cards](EMBED_GLASSESRESEARCH.md) provide a compact Core Report Card for any canonical GLS model. The widget is static, cookie-free, and links readers back to the canonical evidence page.

## Follow new research

- [Research & News](/docs/RESEARCH_NEWS/)
- [RSS feed](https://glassesresearch.org/feed.xml)
- [Verified Research Alerts](/docs/RESEARCH_NEWS/#verified-research-alerts)

## Understand evidence state

`Unknown`, `N/A`, and verified `No` are different states. A missing score or capability is not a negative finding. The [Research Standards](/docs/RESEARCH_STANDARDS/) explain evidence handling, and the [research challenge process](/docs/RESEARCH_CHALLENGES/) is the correction path for stronger or conflicting evidence.

## Technical and historical material

- [Development & Hacking](/hacking/)
- [Industry timeline](/docs/INDUSTRY_TIMELINE/)
- [Technology lineages](/lineages/)
- [Evidence resources](https://glassesresearch.org/data/public/evidence-resources.json)
- [Artifact preservation](/docs/ARTIFACT_PRESERVATION/)
