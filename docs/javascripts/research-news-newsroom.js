(() => {
  ...
  hero.innerHTML = `
    ...
    <p class="gr-newsroom-deck">Verified developments, developing signals, community research and durable analysis — organized by evidence state and research job.</p>
    <div class="gr-newsroom-actions" aria-label="Research and News shortcuts">
      ...
      <a href="/docs/community-research/">Community Research</a>
  ...
  `;
  lead.insertAdjacentElement("afterend", hero);
  ...
  fetch("/data/newsroom-state.json", { credentials: "same-origin" })
    ...
    .then((state) => {
      ...
      if (Array.isArray(state.convergence) && state.convergence.length) {
        ...
        const cards = state.convergence.slice(0, 4).map((theme) => { const supporting = (theme.stories || []).slice(0, 2).map((story) => `<a href="${escapeHtml(story.url)}">${escapeHtml(story.title)}</a>`).join("<span aria-hidden=\"true\"> · </span>"); return `<article class="gr-convergence-card"><div class="gr-signal-row"><span>${escapeHtml(theme.kind)}</span><strong>${escapeHtml(theme.label)}</strong></div><h3>${escapeHtml(theme.story_count)} verified story signals across ${escapeHtml(theme.independent_source_hosts)} source families</h3><p class="gr-convergence-links">${supporting}</p></article>`; }).join("");
  ...
  const loadWireState = async () => {
    ...
    for (const endpoint of ["/wire", "/data/wire-state.json"]) {
      try { const response = await fetch(endpoint, { credentials: "same-origin" }); if (!response.ok) continue; const state = await response.json(); if (state?.schema_version === 1 && Array.isArray(state.items)) { if (state.items.length) return state; emptyState ??= state; } } catch { /* try next read-only source */ }
    }
    if (emptyState) return emptyState;
    throw new Error("wire state unavailable");
  };
  loadWireState().then((state) => { const items = state.items.filter((item) => item && ["reported", "under_review"].includes(item.status) && item.title && item.url).slice(0, 12); if (!items.length || !wireList) return; wireList.innerHTML = items.map(renderWireItem).join(""); wire.hidden = false; }).catch(() => { wire.hidden = true; });
})();
