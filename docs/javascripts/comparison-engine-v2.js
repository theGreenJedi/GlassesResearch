  ...
  Promise.all([
    ...
    .then(([bundle, deviceBundle]) => {
      ...
      const records = devices.map((device) => {
        ...
        return { id: device.id, maker: device.maker, model: device.model, fields };
      });
      ...
      const render = () => {
        ...
        status.textContent = `${left.maker} ${left.model} vs ${right.maker} ${right.model} — ${records.length} models available`;
      };
