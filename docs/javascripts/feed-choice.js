(() => {
  ...
  const FEEDLY_HOME = "https://feedly.com/i/discover";
  ...
  if (verifiedPanel) {
    ...
    if (rssOption) {
      ...
      if (description) description.textContent = "Only verified, published GlassesResearch updates. Copy the RSS URL, then add it through your reader.";
  ...
  if (wireSection) {
    ...
    if (old) {
      old.innerHTML = `<strong>Across the Wire feeds:</strong> <button class="gr-link-button" type="button" data-feed-copy data-feed-url="${WIRE}" data-feed-label="Copy RSS URL">Copy RSS URL</button> · <a href="${FEEDLY_HOME}" target="_blank" rel="noopener noreferrer">Open Feedly</a> · <a href="${INOREADER_HOME}" target="_blank" rel="noopener noreferrer">Open Inoreader</a> · <a href="/docs/FEEDS/#across-the-wire">Reader setup</a> · <a href="${WIRE}">Raw RSS</a> · <a href="/data/wire-feed.json">JSON Feed</a><span data-feed-copy-status role="status" aria-live="polite"></span>`;
    }
  }
