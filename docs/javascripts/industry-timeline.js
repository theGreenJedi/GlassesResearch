(() => {
  ...
  const stateLabels = {
    occurred: 'Occurred', announced: 'Announced', delayed: 'Delayed',
    cancelled: 'Cancelled', provisional: 'Live signal'
  };
    ...
    <div class="timeline-controls" aria-label="Timeline filters">
      ...
      <label>Minimum significance
        <select id="timeline-significance">
          <option value="5">5 — industry-changing</option>
      ...
      <label class="timeline-signal-toggle"><input type="checkbox" id="timeline-signals" checked> Show live primary-source signals</label>
    </div>
  ...
  function showDetail(event) {
    ...
    detail.innerHTML = `<article id="${esc(event.id)}" class="timeline-detail-card">
      <div class="timeline-detail-meta"><span>${esc(event.date)}</span><span>${esc(categoryLabels[event.category] || event.category)}</span><span>Significance ${event.significance}/5</span><span>${esc(stateLabels[event.state] || event.state)}</span></div>
      <h3>${esc(event.title)}</h3><p>${esc(event.summary)}</p><p>${sources}</p>
      <p class="timeline-permalink"><a href="#${esc(event.id)}">Permalink to ${esc(event.id)}</a></p>
    </article>`;
  ...
  function renderChart(events) {
    ...
    events.forEach(event => {
      ...
      circle.setAttribute('aria-label', `${event.date}: ${event.title}; significance ${event.significance} of 5; ${stateLabels[event.state]}`);
  ...
  function renderList(events) {
    ...
    list.innerHTML = events.slice().reverse().map(event => `<article class="timeline-list-item ${eventClass(event)}" id="list-${esc(event.id)}">
      ...
      <p>${esc(event.summary)}</p><div class="timeline-list-meta">${esc(event.id)} · ${esc(categoryLabels[event.category])} · significance ${event.significance}/5 · ${esc(stateLabels[event.state])}</div></div>
    </article>`).join('');
  ...
  function render() {
    ...
    root.querySelector('.timeline-status').textContent = `${events.length} visible milestones · ${canonical.length} canonical events · ${provisional.length} live signals`;
  ...
  Promise.all([
    fetch('/timeline/events.json').then(r => { if (!r.ok) throw new Error('canonical timeline unavailable'); return r.json(); }),
