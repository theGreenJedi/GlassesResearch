...
Brilliant Labs Monocle is almost the opposite of mass-market smart glasses: it is a tiny clip-on heads-up display built explicitly for people who want to inspect, program and modify the device. The hardware combines a 640×400 color OLED, 5 MP camera, microphone, touch controls, Bluetooth and an FPGA, while the software runs an open MicroPython environment with APIs for the display, camera, microphone and touch input. Brilliant also publishes schematics, programming interfaces and guidance for loading custom firmware and FPGA images. The result is not the most discreet or polished everyday eyewear, but it may be one of the purest examples of **smart glasses as a personal computing experiment rather than a vendor-controlled appliance**. Its compromises in battery, ergonomics and consumer polish are real; so is the unusual freedom it gives the owner.

Sources: [Brilliant Labs — Monocle documentation](https://docs.brilliant.xyz/monocle/monocle/), [Brilliant Labs — Monocle hardware](https://docs.brilliant.xyz/monocle/hardware/)
